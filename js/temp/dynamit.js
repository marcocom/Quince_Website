	...
	function setup_logo_block() {
		...
		if(filter) $('header.top .logo .slider').slider({min:0,max:100,value:50,step:1,slide: slider_update,stop: update_grid});
		if(filter) $('header.top .logo').hover(function() {
			$(this).find('.sliders').stop().animate({left:'36px'},Math.floor(animate_speed/2),easing_method);
			$(this).stop().animate({height:'397px'},animate_speed,easing_method);
		},
		function() {
		...
		else $('header.top .logo span.icon').hide();
	...
	function item_actions() {
		...
		$('#grid .blog:hidden a.close').click(function() {
			...
			$(item).css({overflow: 'hidden'}).animate({width:'250px', height:'154px', left:$(item).data('return_left')+'px', top:$(item).data('return_top')+'px'}, animate_speed/2, function() {
				building = false;
				fadein_grid(item);
			});
